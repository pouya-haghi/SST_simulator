 ...
 */
    ...
    SST_ELI_DOCUMENT_PORTS(
        {"port0",  "Link to another component_upDown", { "simpleElementExample.basicEvent", ""} },
        {"port1",  "Link to another component_leftRight", { "simpleElementExample.basicEvent", ""} }
    )
...
private:
    ...
    std::vector<int> predecessors(int id);
    int comp_dir(const int data_id, const std::string op);
    bool check_sent(int id);
    std::map<int, std::vector<int>> communicator_table(int root);
    std::vector<bool> sent; // track which nodes have done sending.
    std::vector<int> sum_local;
    std::map<int, std::vector<int>> comm;

    // Parameters
    int my_id; // added
    int numChildren, dim_row, dim_col; // added
    int numLink;
