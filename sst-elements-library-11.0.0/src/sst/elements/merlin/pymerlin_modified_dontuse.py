...
debug = 0
# print("Hey this is debug", dir(sst))
# print(help(sst))
# print(sst.path)
    ...
    def build(self):
        ...
        for i in range(num_routers):
            ...
            for n in range(_params["torus:local_ports"]):
                ...
                port = port+1
            print("this is router.")
            rtr.enableAllStatistics({"type":"sst.AccumulatorStatistic", "rate":self.statInterval}) #added

    def enableAllStatistics(self,interval):# added
        self.enableAllStats = True;
    ...
    #    print("start id: " + str(id))
        ...
        if level == 0:
            ...
            for l in range(len(host_links)):
                rtr.addLink(host_links[l],"port%d"%l, _params["link_lat"]) # downlinks
            for l in range(len(links)):
                rtr.addLink(links[l],"port%d"%(l+self.downs[0]), _params["link_lat"]) # uplinks
            return
...
#        print("build()")
        ...
        level = len(self.ups)-1 # modified

        print("routers_per_level", self.routers_per_level)
        print("groups_per_level", self.groups_per_level)
        # if level > 1:
...
#                    print("Creating link: link_l%d_g0_r%d_p%d"%(level,i,j))
            ...
            for i in range(self.downs[len(self.ups)-1]): # modified
                self.fattree_rb(level-1,i,group_links[i]) #calling fattree for level 0 (for each of the routers in level 0)
                # if statement in fattree will be executed

            # Create the routers in this level
            radix = self.downs[level] # only down levels
            for i in range(self.routers_per_level[level]):

                rtr_id = self.start_ids[len(self.ups)-1] + i # modified

                rtr = self._instanceRouter(rtr_id,"merlin.hr_router") # for each of the routers in level 1
...
class TestEndPoint(EndPoint):
    ...
    def build(self, nID, extraKeys):
        ...
        if self.group_array is None:
            # num_ep = sst.merlin._params["num_peers"] # added
