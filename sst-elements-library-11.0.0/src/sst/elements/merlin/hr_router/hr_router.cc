    ...
    output(Simulation::getSimulation()->getSimulationOutput())
{
    ...
    // PH: $start - "print busy ports at the startup (constructor)"
    // std::cout << "Start Router: id = " << id << std::endl;
    // for ( int i = 0; i < num_ports; i++ ) {
    //     std::cout << "out_port_busy " << out_port_busy[i] << " in_port_busy " << in_port_busy[i] << std::endl;
    // }
    // std::cout << "End Router: id = " << id << std::endl;
    // PH: $end
    ...
    for ( int i = 0; i < num_ports; i++ ) {
        ...
        // std::cout << "xbar stall[" << i << "] is: " << xbar_stalls[i] << std::endl;
    }
...
hr_router::notifyEvent()
{
    ...
    for ( int i = 0; i < num_ports; i++ ) {
        ...
        // if (in_port_busy[i]!=0)  {std::cout << "in_port_busy[" << i << "] is: " << in_port_busy[i] << std::endl;}
        // if (out_port_busy[i]!=0) {std::cout << "out_port_busy[" << i << "] is: " << out_port_busy[i] << std::endl;}
    }
#endif
    ...
    if ( get_vcs_with_data() == 0 ) {
        ...
        if ( clocking ) {
            if ( arb->isOkayToPauseClock() ) {
                ...
                clocking = false; //PH: pause clocking.
            }
        }
#else
        if ( arb->isOkayToPauseClock() ) {
        ...
        if ( progress_vcs[i] > -1 ) {
            ...
            if ( ev->getTraceType() == SimpleNetwork::Request::FULL ) {
                output.output("TRACE(%d): %" PRIu64 " ns: Copying event (src = %d, dest = %d) "
                              ...
                              " %d, VC %d.\n", // PH: very important with the follwing arguments \
                              // getCurrentSimTimeNano, also we have both input and output VC
                              ev->getTraceID(),
